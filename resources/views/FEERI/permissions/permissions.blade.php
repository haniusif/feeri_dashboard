        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-permission_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
     ...
     @include('FEERI.permissions.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Permissions') }}</h5>
                        @can('create permissions')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('permissions.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('permissions_name') }}</th>
<th  >{{ __('permissions_guard_name') }}</th>
<th  >{{ __('permissions_created_at') }}</th>
<th  >{{ __('permissions_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($permissions as $key => $permission)
            <tr>
            <td>{{ $permission->firstItem + $key + 1 }}</td>
            
<td>{{ $permission->name }}</td>
<td>{{ $permission->guard_name }}</td>
<td>{{ $permission->created_at }}</td>
<td>{{ $permission->updated_at }}</td>         <td>
           <a href="{{ route('permissions.show', $permission->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $permission->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-permission_{{  $permission->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="5"  >{{$permissions->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
