        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-role_has_permission_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Role Has Permissions') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('FEERI.role_has_permissions.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Role Has Permissions') }}</h5>
                        @can('create role_has_permissions')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('role_has_permissions.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('role_has_permissions_permission_id') }}</th>
<th  >{{ __('role_has_permissions_role_id') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($role_has_permissions as $key => $role_has_permission)
            <tr>
            <td>{{ $role_has_permission->firstItem + $key + 1 }}</td>
            
<td>{{ $role_has_permission->permission_id }}</td>
<td>{{ $role_has_permission->role_id }}</td>         <td>
           <a href="{{ route('role_has_permissions.show', $role_has_permission->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $role_has_permission->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-role_has_permission_{{  $role_has_permission->id }}"
                                                    action="{{ route('role_has_permissions.destroy', $role_has_permission->id) }}" method="POST"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="3"  >{{$role_has_permissions->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
