...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
     ...
     @include('FEERI.roles.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Roles') }}</h5>
                        @can('create roles')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('roles.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('roles_name') }}</th>
<th  >{{ __('roles_guard_name') }}</th>
<th  >{{ __('roles_created_at') }}</th>
<th  >{{ __('roles_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($roles as $key => $role)
            <tr>
            <td>{{ $role->firstItem + $key + 1 }}</td>
            
<td>{{ $role->name }}</td>
<td>{{ $role->guard_name }}</td>
<td>{{ $role->created_at }}</td>
<td>{{ $role->updated_at }}</td>         <td>
           <a href="{{ route('roles.show', $role->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $role->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-role_{{  $role->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="5"  >{{$roles->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
