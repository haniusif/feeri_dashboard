...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
     ...
     @include('FEERI.users.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Users') }}</h5>
                        @can('create users')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('users.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<td>
<span class="badge bg-label-{{ ($user->is_active == 1) ? __("success") : __("danger") }} ">
{{ ($user->is_active == 1) ? __("YES") : __("NO") }}
</span>
</td>
<td>
<span class="badge bg-label-{{ ($user->is_verified == 1) ? __("success") : __("danger") }} ">
{{ ($user->is_verified == 1) ? __("YES") : __("NO") }}
</span>
</td>
...
<td>{{ $user->updated_at }}</td>         <td>
           <a href="{{ route('users.show', $user->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $user->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="10"  >{{$users->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
