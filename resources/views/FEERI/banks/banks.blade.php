        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-bank_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Banks') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('FEERI.banks.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Banks') }}</h5>
                        @can('create banks')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('banks.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('banks_name') }}</th>
<th  >{{ __('banks_en_name') }}</th>
<th  >{{ __('banks_is_active') }}</th>
<th  >{{ __('banks_created_at') }}</th>
<th  >{{ __('banks_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($banks as $key => $bank)
            <tr>
            <td>{{ $bank->firstItem + $key + 1 }}</td>
            
<td>{{ $bank->name }}</td>
<td>{{ $bank->en_name }}</td>
<td>
<span class="badge bg-label-{{ ($bank->is_active == 1) ? __("success") : __("danger") }} ">
{{ ($bank->is_active == 1) ? __("YES") : __("NO") }}
</span>
</td>
<td>{{ $bank->created_at }}</td>
<td>{{ $bank->updated_at }}</td>         <td>
           <a href="{{ route('banks.show', $bank->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $bank->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-bank_{{  $bank->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="6"  >{{$banks->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
